 ...
 */
exports.playerClutchStats = function () {
...
exports. playerBioStats = function () {
...
exports.playerCareerStats = function () {
...
nba.stats.allPlayers({  }, (err, res) => {
  ...
  // second loop within loop to parse data and insert into db. 
  for (iloop = 0; iloop < data[0].CommonAllPlayers.length; iloop++) {
    var arr = {}
    arr.group_set = x[iloop]
    var content = data[0].CommonAllPlayers[iloop]
    
    db.query('insert into allPlayers content ' + JSON.stringify(content))
      .then(function (inserted) {
        console.log(inserted)
      })
  }
...
exports.gameLog = function () {
  var sqlQry='select distinct(team_id) as tid from allplayers';
db.query(sqlQry)
  .then(function (teams) {
    for(teamloop=0; teamloop < teams.length; teamloop++) {
      var team = teams[teamloop].tid

nba.stats.teamGamelog({ TeamID: team, Season: '2017-18' }, (err, res) => {
  if (err) {
    console.error(err);
    return;
  }
  ...
  var data=[]
  data.push(q)
  console.log(data[0].TeamGameLog.length)

  
  
  for (iloop = 0; iloop < data[0].TeamGameLog.length; iloop++) {
    var arr = {}
    arr.group_set = x[iloop]
    var content = data[0].TeamGameLog[iloop]
    
    db.query('insert into GameLog content ' + JSON.stringify(content))
      .then(function (inserted) {
        console.log(inserted)
      })
  }
