...
db.query('select from games')
  .then((game) => {
    console.log('db call')
    var playerid = 202323;
    for (game_loop = 0; game_loop < game.length; game_loop++) {
      console.log('loop')
      var gameid = game[game_loop].game_id
      
      var options = { GameID: gameid };
      
      nba.stats.boxscoreAdvanced(options, (err, res) => {
        if (err) {
          console.error(err);
          return;
        }
        ...
        for (x = 0; x < players.length; x++) {
          if (playerid == players[x].player_id) {
            ...
            var updateQry = 'Update Games ADD ' + playername + ' = [' + JSON.stringify(arr) + '] upsert RETURN AFTER @this where game_id = "' + gameid + '"'
            console.log(updateQry)
            db.query(updateQry)
              .then(function (updated) {
                var rid = '#' + updated[0]['@rid'].cluster + ':' + updated[0]['@rid'].position
